...
export default function Home() {
  ...
  const [activeTags, setActiveTags] = useState<Set<Tag>>(new Set()); // NEW
  ...
  const filtered = useMemo(() => {
    ...
    // Tag filter: if any tags are active, include offers that have ALL of them
  ...
  return (
    <div className="min-h-screen text-neutral-900 relative">
      {/* moving light purple/blue bg + top fade (from previous version) */}
      <style jsx global>{`
      ...
      <main className="mx-auto w-full max-w-6xl px-4 sm:px-6 pt-6 pb-16">
        <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white/85 backdrop-blur-sm shadow-sm">
          <div className="grid grid-cols-[2fr_2fr_1fr_140px] items-center px-5 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500 bg-neutral-50/60">
            <div>Casino</div><div>Offer</div><div>Wagering</div><div className="text-right">Bonus</div>
          </div>

          <ul className="divide-y divide-neutral-200">
            {filtered.map((o) => (
              <li key={o.id} className="grid grid-cols-[2fr_2fr_1fr_140px] items-center px-5 py-4">
                <div className="flex items-center gap-3">
